...
#define  streamlog_logger_out( LOGGER, MLEVEL ) (LOGGER).log<streamlog::MLEVEL>().asOstream()
#define  streamlog_logger_message( LOGGER, MLEVEL , CODE_BLOCK , OUT_MESSAGE)\
   if( (LOGGER).wouldWrite< streamlog::MLEVEL >() ) { \
      CODE_BLOCK \
	(LOGGER).log<streamlog::MLEVEL>().asOstream() << OUT_MESSAGE }
//for use in templated classes with gcc 3.2 use these macros:
#define  streamlog_logger_out_T( LOGGER, MLEVEL ) (LOGGER).template log<streamlog::MLEVEL>().asOstream()
